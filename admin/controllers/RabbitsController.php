...
class RabbitsController extends Controller {
    ...
    public function individualweanAction($rabbit_id) {
        ...
        if ($gender == '11')
            $product['rabbit_feeding_group'] = '24';
        else
            $product['rabbit_feeding_group'] = '25';
        ...
        getModel('litter')->wean($litter_id, $product_id);
        if ($litter_count <= 1) {
    ...
    public function notificationAction() {
        ...
        foreach ($rabbits as $r) {
            ...
            if (($rabbit['rabbit_gender'] == 'Female')):
                if (isset($rabbit['rabbit_latest_mate_date']) and ( !isset($rabbit['rabbit_latest_pregnant_date']))): // lateest mate date				
                    $mate_date = new DateTime($rabbit['rabbit_latest_mate_date']);
                    $mate_diff = $today->diff($mate_date)->format("%a");
                    if (11 >= $mate_diff and $mate_diff >= 10) {
                        array_push($mateddays10, $rabbit['product_id']);
                    }
                    if (24 >= $mate_diff and $mate_diff >= 23) {
                        array_push($mateddays23, $rabbit['product_id']);
                    }
                ...
                if ((!isset($rabbit['rabbit_latest_birth_date']) or ! $rabbit['rabbit_latest_birth_date']) and ( isset($rabbit['rabbit_latest_pregnant_date']))):
                    $mate_date = new DateTime($rabbit['rabbit_latest_mate_date']);
                    $mate_diff = $today->diff($mate_date)->format("%a");
                    if (26 >= $mate_diff and $mate_diff >= 25) {
                        array_push($pregnantday13, $rabbit['product_id']);
                    }
                    if (31 >= $mate_diff and $mate_diff >= 30) {
                        array_push($pregnantday18, $rabbit['product_id']);
                    }
                ...
                if (isset($rabbit['rabbit_latest_birth_date'])):
                    ...
                    if (20 >= $birth_diff and $birth_diff >= 19) {
                        array_push($weaning2days_before, $rabbit['product_id']);
                    }
                    if (27 >= $birth_diff and $birth_diff >= 26) {
                        array_push($weaning2days_after, $rabbit['product_id']);
                    }
                endif;
            //End of weaning
        ...
        if (isset($mateddays10)) {
            $data['mateddays10'] = $mateddays10;
        }
        if (isset($mateddays23)) {
            $data['mateddays23'] = $mateddays23;
        }
        if (isset($pregnantday13)) {
            $data['pregnantday13'] = $pregnantday13;
        }
        if (isset($pregnantday18)) {
            $data['pregnantday18'] = $pregnantday18;
        }
        if (isset($weaning2days_before)) {
            $data['weaning2days_before'] = $weaning2days_before;
        }
        if (isset($weaning2days_after)) {
            $data['weaning2days_after'] = $weaning2days_after;
        }
