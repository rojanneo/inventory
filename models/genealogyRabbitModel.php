...
class GenealogyRabbitModel extends Model
{
  ...
  public function rabitparent($r_id)
  {
    ...
    if($pdetail){ $pdetail=$pdetail[0]; return $pdetail; }
    else{ return false;}
  }
...
{ 
  ...
  if(empty($xinformation))  { return'No rabbit with ID ='.$x.' exsit'; } else { $xDetail=array(); $xDetail['buck_id']=$xinformation['buck_id']; $xDetail['does_id']=$xinformation['does_id'];}
  $yinformation=$this->rabitparent($y);
  if(empty($yinformation))  { return'No rabbit with ID ='.$y.' exsit'; } else { $yDetail=array(); $yDetail['buck_id']=$yinformation['buck_id']; $yDetail['does_id']=$yinformation['does_id'];}
  ...
  if($xinformation['type']==$yinformation['type'])     { return'Both Have Same Gender'; } 
  if($xinformation['status']==0) { return'Sorry Rabbit with ID'. $x.'Status is 0. It either mean this rabbit is death or is already sold as a product'; }
  if($yinformation['status']==0) { return'Sorry Rabbit with ID'. $y.'Status is 0. It either mean this rabbit is death or is already sold as a product'; }
  ...
  if(empty($intersectionvalue)){ return'Yes This Rabbit Can Be Mated';}
  else 
  { 
...
public function recursionparentfinder($xDetail,$generationupto)
{
  ...
  */
  ...
  if($xDetail['buck_id']) { $xmaleparents[1]  =$xDetail['buck_id']; }
  if($xDetail['does_id']) { $xfemaleparents[1]=$xDetail['does_id'];   } 
  for($i=3;$i<$generationupto;$i++)
  {
    ...
    foreach ($xDetail as $xinfoLoop) 
    {
     ...
     if($mf)
     { 
      if((isset($mf['buck_id']))||($mf['buck_id']!=0)){ $xmaleparents[$j]=$mf['buck_id']; $xmf[$k]=$mf['buck_id']; $k++;  }
      if((isset($mf['does_id']))||($mf['does_id']!=0)){ $xfemaleparents[$j]=$mf['does_id']; $xmf[$k]=$mf['does_id']; $k++;}        
      $j++;          
     }      
    }
    if(isset($xmf)){ unset($xDetail);   $xDetail=$xmf;    }    
  }
...
public function intersectioncheck($xinfo,$yinfo)
{
  ...
  foreach ($xinfo['male'] as $xmale)
  {
    foreach ($yinfo['male'] as $ymale) 
    {
      ...
      if($intersectedvalue){ $intersectedarrayvalue[$k]=$intersectedvalue; $k++; }      
    }    
  }
...
public function insertrabbit($post_data)
...
{
  if(empty($comparevalue) && empty($comparecolumn)){  $query='SELECT `'.$element.'` FROM `'.$table.'`'; }
  else {  $query='SELECT `'.$element.'` FROM `'.$table.'` WHERE `'.$comparecolumn.'`="'.$comparevalue.'" ';}
  $sql=$this->connection->Query($query); 
  if($sql){ return $sql; }
    else return false;
}
...
public function comparevalue($does_id,$buck_id)
{
  ...
  if(empty($xinformation))  { return false; } else { $xDetail=array(); $xDetail['buck_id']=$xinformation['buck_id']; $xDetail['does_id']=$xinformation['does_id'];}
  $yinformation=$this->rabitparent($buck_id);
  if(empty($yinformation))  { return false; } else { $yDetail=array(); $yDetail['buck_id']=$yinformation['buck_id']; $yDetail['does_id']=$yinformation['does_id'];}
  if(($xDetail['buck_id']==0) && ($xDetail['does_id']==0) || ($yDetail['buck_id']==null) && ($yDetail['does_id']==null)) // If the parent is null then they can mate with anyone
  {
    return true;
  }
 ...
 $yinfo=$this->recursionparentfinder($yDetail,7);
 $intersectionvalue=$this->intersectioncheck($xinfo,$yinfo);
 if(empty($intersectionvalue)){ return true;}
 else {  return false; }
}
